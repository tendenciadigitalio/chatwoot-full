...
read -p "🌐 URL del panel Portainer (ej. https://portainer.midominio.com): " PORTAINER_URL
...
if [ "$JWT" == "null" ] || [ -z "$JWT" ]; then
  ...
  exit 1
fi -H "Content-Type: application/json" -d "{"Username": "${PORTAINER_USER}", "Password": "${PORTAINER_PASS}"}" | jq -r .jwt)


EXISTING_STACK_ID=$(curl -sk -H "Authorization: Bearer $JWT" "$PORTAINER_URL/api/stacks" | jq ".[] | select(.Name==\"chatwoot-${CLIENT_ID}\") | .Id")
